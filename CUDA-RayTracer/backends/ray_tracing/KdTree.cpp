...
Color KdTree::trace(Vector vector, int depth)
{
	...
	for (int i = num - 1; i >= 1; i--)
	{
		...
		for (int light = 0; light < numberOfLights; ++light)
		{
			...
			if (index != -1 && (scene->getTriangles()[index].getDist(to_light) < lights[light].point.getDist(reflection_point))) // fix this
			{
				continue;
			}
			...
			Vector from_light_reflected = scene->getTriangles()[triangles[i]].getReflectedVector(from_light);
			...
			triangle_ilumination += lights[light].Is * powf(std::max(0.f, to_viewer.dot(from_light_reflected)),
			                                                material.specularExponent) * material.specular;
		}

		if (i < num - 1)
		{
			triangle_ilumination += res * powf(std::max(0.f, to_viewer.dot(normal)), material.specularExponent) * material.
				specular;
		}
...
bool KdTree::split(std::vector<int>& triangles, std::vector<int>& left, std::vector<int>& right, int axis)
{
